    ...
    <div class="banner">
      <div class="banner-imgs">
        <a
          ...
          :key="key"
          href="javascript:;"
          @click="jumpNews(item.content0)"
        >
          <img :src="item.facePictureId[0]" alt v-show="key === index">
        </a>
      </div>
          ...
          <div class="one-change" @mouseover="stopTimes" @mouseout="startTimes">
            <swiper :options="swiperOption" ref="mySwiper">
              ...
              <swiper-slide v-for="(item,key) in data2" :key="key">
                <img
                  :src="item.facePictureId[0]"
                  alt
                  :class="{imgActive:index === key}"
                  @click.stop="jumpImg(key)"
                >
              </swiper-slide>
              <!-- Optional controls -->
            </swiper>
          ...
          <el-col :span="12" class="info-col1">
            ...
            <div class="info-img2 clearDiv">
              ...
              <div class="info-p">
                <p>
                  {{$t('index.Ticket1')}}
                  <span>{{ticketPrice.content0 ? ticketPrice.content0.substring(3,ticketPrice.content0.length-4) : ''}}</span>
                  /{{$t('index.person')}}
                </p>
                <p class="p">
                  {{$t('index.Ticket2')}}
                  <span>{{ticketPrice.content1 ? ticketPrice.content1.substring(3,ticketPrice.content1.length-4) : ''}}</span>
                  /{{$t('index.person')}}
                </p>
              </div>
            </div>
